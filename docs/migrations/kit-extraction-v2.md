...
The initial ignored inventory contained the SPEC, root third-party notice, empty `docs/`, `.tmp-wsl-dashboard/`, `legacy/`, and `target/`. No root AGENTS file was present. Existing legacy source, historical visual baselines, and build outputs are retained in place; they are not part of the Git bundle. They must be protected before any later physical relocation. No old docs or agent instructions were used as architectural references.
...
| `legacy/dotnet-reference` | RETAINED | Historical source is not established to be disposable; no deletion. |
